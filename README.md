...
|   김민석  |   	|
|   홍재민	|   	|
|   임현조	|  처음으로 Vue.js를 사용해 REST API를 통해 서버와 데이터를 주고받는 과정이나 컴포넌트의 생명 주기를 고려해 메서드를 작성하는 데 어려움을 느꼈지만, 팀원들이 먼저 기반을 잘 마련해준 덕분에 맡은 부분을 무사히 완성할 수 있었습니다. 프론트엔드 개발 중 오류를 하나씩 해결할 때마다 큰 성취감을 느꼈고, 동시에 프론트엔드 기술에 대한 부족함도 많이 느껴 더 깊이 배우고 싶다는 열정을 갖게 되었습니다. 	|
