...
|   김민석  |   	|
|   홍재민	|   	|
|   임현조	|  처음으로 vue.js를 사용해서 rest api로 서버와 데이터를 주고 받는 법이나 컴포넌트 생명 주기를 생각해서 함수를 만들어야 함에 어려움을 느꼈지만 팀원들이 먼저 기반을 만들어줘서 맡은 부분을 완성할 수 있었습니다. 이번 프로젝트에서 프론트 엔드에 오류를 해결할 때마다 성취감과 동시에  프로트엔드 기술에 대한 부족함을 많이 느꼈고 더 배우고 싶은 열정을 가지게 되었습니다. 	|
