                    ...
                    <form @submit.prevent="submitPostData">
                        <div class="grid grid-cols-1 gap-6 mt-4">
                            <div>
                                ...
                                <input type="text" v-model="teamName.value" class="w-full mt-2 border-gray-200 rounded-md focus:border-indigo-600 focus:ring focus:ring-opacity-40 focus:ring-indigo-500" />
                            </div>
                            <div>
                                <label class="text-gray-700" for="teamIntroduce">내용</label>
                                <textarea v-model="teamIntroduce.value" class="w-full mt-2 border-gray-200 rounded-md focus:border-indigo-600 focus:ring focus:ring-opacity-40 focus:ring-indigo-500"></textarea>
                            </div>
                            <div class="form-group">
                                <label class="text-gray-700">상태</label>
                                <select v-model="projectStatus.value" class="w-full mt-2 border-gray-200 rounded-md focus:border-indigo-600 focus:ring focus:ring-opacity-40 focus:ring-indigo-500">
...
const submitPostData = async () => {
    ...
    try {
        if (isEditMode.value) {
            await apiClient.put(`/team/${teamNo.value}`, params, config);
            alert("게시글이 수정되었습니다.");
            router.push(`/team/${teamNo.value}`);
        } else {
            const response = await apiClient.post("/team", params, config);
            alert("팀이 생성되었습니다.");
